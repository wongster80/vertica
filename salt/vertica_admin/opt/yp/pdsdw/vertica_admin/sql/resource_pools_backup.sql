...
SELECT    'CREATE RESOURCE POOL ' || name
        ...
        || CASE WHEN cpuaffinityset            IS NULL THEN ' ' ELSE ' CPUAFFINITYSET '                 || '''' || cpuaffinityset               || '''' END
        || CASE WHEN cpuaffinitymode           IS NULL THEN ' ' ELSE ' CPUAFFINITYMODE '                 || '' || cpuaffinitymode               || '' END
        || ' ; '
